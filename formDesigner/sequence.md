...
- 当某个序号的时间维度设置为`天`时，当日生成的序号都是从1开始自增。
- 设置为`周`，则本生成的序号都是从1开始自增，下一周起序号重新从1开始自增。
...
表示序号，参数表示要显示的序号位数，解析后依据规则会替换成序号值。以下示例假设当前生成的序号为100。
...
序号字段值的显示位数，当生成的序号位数小于设置的固定位数时，在序号之前使用0补全到固定位数显示。比如固定位数设置为6，生成的序号值为100，最终表单序号字段的值显示为`000100`。
